...
const Signin = ({ setSignUpOpen, setSignInOpen }) => {
  ...
  const handleLogin = async (e) => {
    ...
    if (!disabled) {
      ...
      // console.log("hehehe");
      try {
        const res = await axios.post("https://cast-wave-api.vercel.app/api/auth/signin", {
          email: email,
          password: password,
        });
        if (res.status === 200) {
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        ...
        // console.log(user);
        const res = await axios.post("https://cast-wave-api.vercel.app/api/auth/google", {
          name: user.data.name,
    ...
    if (optVerified) {
      ...
      try {
        const res = await axios.put(
          "https://cast-wave-api.vercel.app/api/auth/forgetPassword",
          {
            email,
    ...
    if (!resetDisabled) {
      ...
      try {
        const res = await axios.get(
          `https://cast-wave-api.vercel.app/api/auth/findByEmail?email=${email}`
        );
        if (res.status === 200) {
